...
import (
	...
	"homework-1/loms/internal/app/stock"
...
type Service struct {
	...
	stockService stock.StockServiceInterface
}

func New(orderRepo order.OrderInterface, stockService stock.StockServiceInterface) *Service {
	return &Service{
		orderRepo:    orderRepo,
...
func (s *Service) OrderCreate(ctx context.Context, request *pb.OrderCreateRequest) (*pb.OrderCreateResponse, error) {
	...
	if err != nil {
		...
		if err = s.orderRepo.SetStatus(ctx, orderID, constants.StatusFailed); err != nil {
			return nil, err
		}
	}
...
func (s *Service) OrderPay(ctx context.Context, request *pb.OrderPayRequest) (*pb.OrderPayResponse, error) {
	...
	for _, item := range items {
		if err = s.stockService.ReserveRemove(ctx, item.SKU, item.Count); err != nil {
			return nil, err
		}
	}
...
func (s *Service) OrderCancel(ctx context.Context, request *pb.OrderCancelRequest) (*pb.OrderCancelResponse, error) {
	...
	for _, item := range items {
		if err = s.stockService.ReserveCancel(ctx, item.SKU, item.Count); err != nil {
			return nil, err
		}
	}
...
func (s *Service) StocksInfo(ctx context.Context, request *pb.StocksInfoRequest) (*pb.StocksInfoResponse, error) {
	...
	count, err := s.stockService.GetBySKU(ctx, request.Sku)
	if err != nil {
		return nil, err
	}
...
func convertItems(protoItems []*pb.OrderItem) []model.OrderItem {
...
func backConvert(orderItems []model.OrderItem) []*pb.OrderItem {
