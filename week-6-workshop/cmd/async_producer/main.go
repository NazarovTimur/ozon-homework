...
import (
	...
	"gitlab.ozon.dev/go/classroom-14/students/week-6-workshop/internal/domain/order"
	"gitlab.ozon.dev/go/classroom-14/students/week-6-workshop/internal/infra/kafka/async_producer"
)
...
func main() {
	...
	prod, err := async_producer.NewAsyncProducer(conf.KafkaConfig,
		...
		//async_producer.WithProducerFlushMessages(2),
		async_producer.WithProducerFlushFrequency(5*time.Second),
		//async_producer.WithProducerFlushFrequency(20*time.Second),
	)
	if err != nil {
		log.Fatal(err)
	}
